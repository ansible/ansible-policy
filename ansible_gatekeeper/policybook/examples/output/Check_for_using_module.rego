...
Check_for_module_name = true if {
    not input.task.module_fqcn in allowed_modules
} else = false


deny = true if {
    Check_for_module_name
    print(sprintf("The package %v is not allowed, allowed modules are one of %v", [input.task.module_fqcn, allowed_modules]))
} else = false
